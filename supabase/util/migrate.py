...
def replies_after_date(timestamp: datetime):
    events_after_today = firestore_client.collection('events').where(
        filter=FieldFilter('timestamp', '>=', timestamp)).stream()
...
replies = pd.DataFrame(replies_after_date(TIMESTAMP), columns=['schedule_id', 'instance_date', 'attendee_id', 'selected_option'])
...
members = invitees[['displayName', 'group_id']].rename(columns={'displayName': 'display_name_override'}).to_dict('records')
...
map_phone_to_member_id = {invitee['phoneNumber']: invitee['member_id'] for member_id, invitee in invitees.iterrows()}
replies['member_id'] = replies['attendee_id'].map(map_phone_to_member_id).astype('Int64')
...
# }
map_schedule_id_to_supabase_id = {
    'AGvmeGq4sVMntxmIBhJn': 1, # Pranzo
    '9dA4LAjp5hjh3bQ82S4p': 2, # Cena
}
replies['schedule_id'] = replies['schedule_id'].map(map_schedule_id_to_supabase_id)

supabase_client.table('replies').upsert(replies[['member_id', 'schedule_id', 'instance_date', 'selected_option']].dropna().to_dict('records')).execute()


invites = invitees[['member_id', 'phoneNumber']].rename(columns={'phoneNumber': 'value'})
...
invites_with_codes['value'] = invites_with_codes['member_id'].apply(lambda _: generate_invite_code())
