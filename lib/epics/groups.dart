...
Stream<dynamic> _loadGroupsOnInviteCodeUseEpic(
  Stream<dynamic> actions,
  EpicStore<AppState> store,
) => actions.whereType<SuccessUseInviteCode>().map(
  (event) => const RequestRetrieveAll<Group>(),
);
...
Stream<dynamic> _loadGroupOnGroupDetailsOpenEpic(
  Stream<dynamic> actions,
  EpicStore<AppState> store,
) => actions.whereType<GroupDetailsOpenAction>().map(
  (action) => RequestRetrieveOne<Group>(action.groupId),
);
        ...
        (action) => groups
            ...
            .expand(
              (userGroups) => [
                ...
                SuccessRetrieveMany(
                  userGroups.profiles.toList(growable: false),
                ),
              ],
            ),
      );
...
Epic<AppState> _createCreateOneGroupEpic(
  ...
  return (Stream<dynamic> actions, EpicStore<AppState> store) =>
      actions.whereType<CreateGroupAction>().asyncMap((action) async {
        final picture = action.image != null
            ? await storage.uploadPublicXFile(const Uuid().v7(), action.image!)
            : null;
        ...
        return groups
            ...
            .catchError((error) => FailCreateOne(entity: group, error: error));
      });
}
...
Epic<AppState> _createUpdateOneGroupEpic(
  ...
  return (Stream<dynamic> actions, EpicStore<AppState> store) =>
      actions.whereType<UpdateGroupAction>().asyncMap((action) async {
        final picture = action.image != null
            ? await storage.uploadPublicXFile(const Uuid().v7(), action.image!)
            : null;
        ...
        return groups
            ...
            .catchError((error) => FailUpdateOne(entity: group, error: error));
      });
}
...
Epic<AppState> _createDeleteOneGroupEpic(GroupsRepository groups) {
  return (Stream<dynamic> actions, EpicStore<AppState> store) =>
      actions.whereType<RequestDeleteOne<Group>>().asyncMap(
        (action) => groups
...
Stream<dynamic> _navigateToHomePageEpic(
  Stream<dynamic> actions,
  EpicStore<AppState> store,
) => actions.whereType<SuccessDeleteOne<Group>>().map(
  (action) => NavigateReplaceAction(HomeScreenRoute().location),
);
