  ...
  return (Stream<dynamic> actions, EpicStore<AppState> store) =>
      actions.whereType<GroupDetailsOpenAction>().asyncMap(
        (action) => replies
            ...
            .then<dynamic>(
              (replies) => SuccessRetrieveMany(replies.toList(growable: false)),
            )
...
Epic<AppState> _createRequestDeleteReplyEpic(RepliesRepository replies) {
  return (Stream<dynamic> actions, EpicStore<AppState> store) =>
      actions.whereType<RequestDeleteReplyAction>().asyncMap((action) {
        ...
        if (replyId == null) {
          return FailDeleteOne<Reply>(
            id: "$memberId-$scheduleId-$instanceDate",
            error: "No ID",
          );
        }
        return replies
