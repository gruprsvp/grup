...
Epic<AppState> _createRetrieveMembersEpic(ProfilesRepository profiles) {
  return (Stream<dynamic> actions, EpicStore<AppState> store) =>
      actions.whereType<SuccessRetrieveMany<Member>>().flatMap(
        (action) =>
            Stream.fromIterable(action.entities).map((member) {
              final profile = member.profile;
              if (profile == null) return null;

              return SuccessRetrieveOne<Profile>(profile);
        ...
        final picture =
            image != null
                ...
                : store
                    .state
                    .profiles
                    .entities[store.state.auth.user!.id]
                    ?.picture;
