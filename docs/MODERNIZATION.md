...
> **D1 update (2026-08-07):** the user has green-lit "completely changing the state management and patterns if needed" to align with current Flutter best practices, as long as the app stays thoroughly tested and reliable. That resolves D1's direction: **Path B (migrate) is approved in principle** — but it stays *sequenced* behind the safety net it depends on. Order of operations: merge Phase 0 (PR #214) → ship the isolated Phase 1 wins → do Phase 1.5 (failure-path correctness) *as part of* the migration design rather than twice — then execute the migration slice-by-slice behind the existing repository interfaces. Riverpod is the presumptive target (first-party async patterns, memoized providers, wide ecosystem); confirm against the Flutter team's current guidance when the migration slice starts.
...
**[A2] Security — close the invite perimeter** — **DONE 2026-08-07** (branch `fix/invite-perimeter`, stacked on Phase 0). All four items below implemented: `Random.secure()` codes; EXECUTE perimeter migration (`20260807150000`, incl. `alter default privileges` so future functions aren't PUBLIC-executable); phone format canonicalized to E.164-with-`+` in `handle_new_invite` (`20260807150200`), phone signup enabled locally (`config.toml` `[auth.sms]` + dummy Twilio, no SMS ever sent — autoconfirm), live phone test un-skipped; member self-service policies + `handle_member_left` admin-continuity trigger (`20260807150100` — promotes the oldest claimed-profile member when the last admin leaves, deletes unmanageable groups). pgTAP grew 37 → 64 assertions (`function_grants_test.sql`, `invite_claim_test.sql`, write-policy flips). _Residual (fold into later passes): the contacts picker still sends raw `p.number` un-normalized (`widgets/group_members.dart:175`) — E.164-normalize at the UI boundary; `handle_new_invite` doesn't reset `display_name_override` while `handle_upsert_user` does._
...
- **Dependency cleanup** — **largely DONE 2026-08-07** (branch `chore/dependency-upgrades`): all direct deps at latest resolvable (majors taken: `google_fonts` 8, `flutter_contacts` 2 — API migration in `select_contacts.dart`, `app_links` 7, `device_info_plus` 13, `package_info_plus` 10, `share_plus` 13 — migrated to `SharePlus.instance.share`, `styled_text` 10, `supabase_auth_ui` 0.7, `supabase_flutter` 2.17). **`supabase_flutter` 2.17 dropped the `passkeys` dependency** — the Corbado `bundle.js` hack is removed from `web/index.html`; the iOS 16 / macOS 13.5 floors it forced could now be revisited (lowering iOS back below 16 would restore iOS 12–15 users — product decision, floors kept for now). Removed dead weight: `flutter_flavorizr` (declared but never configured; it was the solver blocker for three majors) and `supabase/util/` (completed one-time Firestore→Supabase migration script whose `poetry.lock` was the source of **all 51 Dependabot alerts**). Contacts picker now prefers `Phone.normalizedNumber` (E.164). Remaining: `intl`/`build_runner`/`test` are SDK-pinned; redux-family dormancy unchanged (D1). Original notes: pin `supabase_auth_ui` fork to a SHA (it's on a moving `ref: main`, 31 commits behind a still-maintained upstream) or rebase customizations onto upstream; fix the false `redux_entity` "overridden below" comment; plan the `styled_text` 8→9 breaking bump. **[A2] 2026-07-09 status:** the fork is gone (upstream 0.6.1, no overrides — done); 7 direct deps now have pending **major** bumps (`google_fonts` 6→8, `flutter_contacts` 1→2, `app_links` 6→7, `device_info_plus`, `package_info_plus`, `share_plus`, `styled_text` 8→9); none of the redux family is formally discontinued on pub.dev, but `redux`/`flutter_redux`/`redux_persist`/`reselect` last published **2021–2022** — consistent with D1's framing.
