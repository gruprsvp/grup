...
### Remaining Phase 0 _(updated 2026-07-09 by the second audit)_
- ~~**Split hermetic vs live-Supabase tests**~~ **Done** (`dart_test.yaml` `live` tag, `@Tags` + self-skip in `repositories_test.dart`) — **but defeated in practice** until `supabase/config/localhost.json` is un-tracked (`git rm --cached`): the file ships with every clone, the existence-based self-skip never fires, and `flutter test` without Docker fails. See [Phase 0 addendum item 7](#phase-0-addendum-a2--holes-found-in-the-safety-net-itself-2026-07-09).
- **De-risk `build.yaml`** (gate prod `db push`, add iOS build) — _production-sensitive, propose before merging._ **[A2] expanded scope:** also concurrency group, `db push` after the release gate, `permissions:` blocks, SHA-pinned actions, dependabot `github-actions` ecosystem, `timeout-minutes` (addendum item 10).
- **[A2] Close the verify.yaml draft-gate bypass** (`ready_for_review` trigger — addendum item 8).
- **[A2] Widen pgTAP + characterization coverage to the risky paths** (write policies + `*_all_self` + storage; non-UTC/DST/month-boundary recurrence — addendum item 9).
